...
def validate_chunk(
    ...
    chunk_id = chunk_data["chunk_id"].strip()
...
def load_chunks(
    ...
    json_files = sorted([f for f in input_path.iterdir() if f.is_file() and f.name.endswith(".json")])
    if not json_files:
...
def generate_embeddings(
    ...
    """
    Tạo Gemini embedding cho danh sách chunk. Hỗ trợ truyền client để mock/test.
    """
    ...
    for chunk in chunks:
        ...
        try:
            ...
            raise ValueError(f"Lỗi tạo embedding Gemini cho chunk_id '{chunk['chunk_id']}': {e}")
...
def run_query(
    ...
    try:
        ...
        generation_failed = True
        # Làm sạch thông báo lỗi, tránh lộ secret/key
    ...
    # Tìm tất cả nhãn trích dẫn dạng [E1], [E2], ...
    ...
    for label_num in found_labels:
        ...
        if ev_id in accepted_map:
            ...
            # Thay thế nhãn [E1] bằng chuỗi display
            ...
            # Nhãn không tồn tại trong accepted evidence (ví dụ [E99]) -> Loại bỏ khỏi answer và thêm warning
    ...
    # Làm sạch khoảng trắng thừa do thay thế/xóa nhãn
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Buổi 07 - RAG Pipeline System")
    ...
    validate_parser.add_argument(
        ...
        help="Chiến lược chunking cần validate (default: hierarchical)"
    )
    validate_parser.add_argument(
        "--input-dir",
        type=str,
    ...
    status_parser.add_argument(
        ...
        default="hierarchical",
        choices=sorted(list(ALLOWED_STRATEGIES)),
        help="Chiến lược chunking cần kiểm tra status (default: hierarchical)"
    )
    ...
    index_parser.add_argument(
        ...
        default="hierarchical",
        choices=sorted(list(ALLOWED_STRATEGIES)),
        help="Chiến lược chunking cần index (default: hierarchical)"
    )
    index_parser.add_argument(
        "--input-dir",
        type=str,
    ...
    index_parser.add_argument(
        "--reset",
        action="store_true",
        help="Reset/xóa collection cũ trước khi re-index"
    )
    ...
    query_parser.add_argument(
        ...
        type=int,
        default=5,
        help="Số lượng evidence truy xuất tối đa (default: 5)"
    )
